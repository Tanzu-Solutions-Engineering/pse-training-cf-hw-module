...
. Create an application manifest in +manifest.yml+ (substitute your application domain):
+
[source,yml]
----
---
...
. Add the following to +src/main/resources/application.properties+ to cause Hibernate to create the database schema and import data at startup (his is done automatically for embedded databases, not for custom ++DataSource++s. Other Hibernate native properties can be set in a similar fashion):
+
[source,java]
----
